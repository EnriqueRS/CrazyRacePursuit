...
public class Figura {
    ...
    public ArrayList<simulador.Figura> listaObjetosFisicos;
    ...
    boolean coche;
    boolean activarPerseguir;
    boolean frenar;
    public List<BackgroundSound> sonido;
    boolean camara;

    public Figura(BranchGroup conjunto, ArrayList<Figura> listaObjetosFisicos, Juego juego, Boolean coche) {
    ...
    protected void anadirSonidoARama(BranchGroup b, String soundFile, BoundingSphere limites, int indice) {
...
//Create the background sound from the media container
        sonido.add(new BackgroundSound(droneContainer,
                1.0f));
//Activate the sound
        sonido.get(indice).setSchedulingBounds(limites);
        sonido.get(indice).setCapability(PointSound.ALLOW_ENABLE_READ);
        sonido.get(indice).setCapability(PointSound.ALLOW_ENABLE_WRITE);
        sonido.get(indice).setCapability(PointSound.ALLOW_LOOP_WRITE);
        //drone.setEnable(true);
//Set the sound to loop forever
        //drone.setLoop(BackgroundSound.INFINITE_LOOPS);
        b.addChild(sonido.get(indice));
    }
    ...
    public void cambiarMov() {
        //ab.playAnimation( "iron_golem:cwalk", true );
    }

    public void cambiarModelo(String archivo) {
    }
    ...
    public void actualizar() {
        ...
        if (localizacionObjetivo != null) {
            ...
            fuerzaDePersecucion = new Vector3f(direccion.x * masa * aceleracionMuscular / 4f, 0, direccion.z * masa * aceleracionMuscular / 4f);  //Crea vector fuerza
            cuerpoRigido.applyCentralForce(fuerzaDePersecucion);//aceleracionMuscular/2f,
        }
