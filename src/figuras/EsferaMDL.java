...
public class EsferaMDL extends Figura {
    ...
    boolean activarCorre;
    boolean activarParado;

   public EsferaMDL(String ficheroMDL, float radio,  BranchGroup conjunto,  ArrayList<Figura> listaObjetos, Juego juego, boolean esPe){
      super(conjunto,  listaObjetos, juego, false);
      esMDL=true;
      esPersonaje = esPe;
...
TransformGroup crearObjetoMDL(String archivo, float multiplicadorEscala){
    ...
    public void cambiarMov() {
        if (this.corriendo && !this.activarCorre) {
            activarCorre = true;
            activarParado = false;
            ab.playAnimation( "iron_golem:cwalk", true );
        } else if (!this.corriendo && !this.activarParado) {
            activarCorre = false;
            activarParado = true;
