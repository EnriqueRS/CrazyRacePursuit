 ...
 * Enrique Rios Santos
 */
public class Salto extends Figura{
    
    public Salto(BranchGroup conjunto, ArrayList<Figura> listaObjetosFisicos, Juego juego) {
        super(conjunto, listaObjetosFisicos, juego, false);
        
        //Creando una apariencia
        Appearance apariencia = new Appearance();
        Texture tex = new TextureLoader(System.getProperty("user.dir") + "//Texturas//metal.jpeg", juego).getTexture();
        ...
        Box rampaVisual = new Box(5f,grosor,tam,Box.GENERATE_TEXTURE_COORDS,apariencia);//2.5f
        BoxShape rampaFisica = new BoxShape(new Vector3f(5f,grosor,tam)); //2.5f 
        DibujarCollisionShape.dibujarBoxShape(bg, rampaFisica);//dibujar figurafisica
        
        ramaFisica = new CollisionObject();
        ramaFisica.setCollisionShape(rampaFisica);
        bg.addChild(rampaVisual);
    ...
    public void aplicarRotacion(float alpha,float masa, float elasticidad, float dumpingLineal, float posX, float posY, float posZ, DiscreteDynamicsWorld mundoFisico) {
        ...
        rot.set(new Vector3f(1, 0, 0), alpha);
